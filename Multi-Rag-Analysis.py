...
class RAGSystems:
    ...
    def _evaluate_naive_rag(self, question, ground_truth, k=3):
        ...
        try:
            ...
            retrieval_end_mem = abs(psutil.virtual_memory().used / (1024 * 1024))
            retrieval_mem = abs(retrieval_end_mem - retrieval_start_mem)
    ...
    def _evaluate_graph_rag(self, question, ground_truth, k):
        ...
        try:
            ...
            gen_end_mem = psutil.virtual_memory().used / (1024 * 1024)
            gen_mem = abs(gen_end_mem - gen_start_mem)
            gen_time = time.time() - gen_start_time
    ...
    def _evaluate_hybrid_rag(self, question, ground_truth, k=3):
        # Initialize as dictionary first
        evaluation_result = {
            "answer": "",
            "context": "",
            "metrics": {
        ...
        try:
            # Phase 1: Measure retrieval (vector + graph)
            retrieval_start_time = time.time()
            retrieval_start_mem = psutil.Process().memory_info().rss / (1024 * 1024)  # MB
            ...
            # Graph expansion - store query result in separate variable
            graph_chunks = []
            if self.neo4j_driver:
                with self.neo4j_driver.session() as session:
                    ...
                    LIMIT $double_k
                    """, ids=top_ids, double_k=k*2)
                    graph_chunks = [record["text"] for record in neo4j_result]
            
            retrieval_end_mem = psutil.Process().memory_info().rss / (1024 * 1024)
            retrieval_time = time.time() - retrieval_start_time
            retrieval_mem = abs(retrieval_end_mem - retrieval_start_mem)

            # Combine contexts
            chroma_context = "\n\n".join(chroma_results['documents'][0])
            graph_context = "\n\n".join(graph_chunks) if self.neo4j_driver else ""
            combined_context = f"Vector Results:\n{chroma_context}\n\nGraph Results:\n{graph_context}"
            evaluation_result['context'] = combined_context

            # Phase 2: Measure generation
            gen_start_time = time.time()
            gen_start_mem = psutil.Process().memory_info().rss / (1024 * 1024)
            ...
            gen_end_mem = psutil.Process().memory_info().rss / (1024 * 1024)
            gen_time = time.time() - gen_start_time
            gen_mem = max(0, gen_end_mem - gen_start_mem)

            # Calculate metrics
            relevant_chunks = self._get_relevant_chunks(question, ground_truth, top_ids)
            precision_at_k, recall_at_k = self._calculate_precision_recall_at_k(
                top_ids, relevant_chunks, k)

            bleu_score = self._calculate_bleu(answer, ground_truth)
            f1 = self._calculate_f1(answer, ground_truth)
            hallucination_rate = self._detect_hallucinations(answer, combined_context)

            # Update results
            evaluation_result['metrics'].update({
                "precision@k": precision_at_k,
                "recall@k": recall_at_k,
                "f1_score": f1,
                "bleu_score": bleu_score,
            ...
            evaluation_result['answer'] = f"Error: {str(e)}"
